...
def resnet_model_fn(features, labels, mode, model_class,
                    resnet_size, weight_decay, learning_rate_fn,
                    data_format, loss_filter_fn=None, params=None):
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    learning_rate = params['learning_rate'] # learning_rate_fn(global_step)

    # Create a tensor named learning_rate for logging purposes
    tf.identity(learning_rate, name='learning_rate')
    tf.summary.scalar('learning_rate', learning_rate)

    learn_rule = params['learning_rule']
    if learn_rule == 'sgd':
      optimizer = tf.train.GradientDescentOptimizer(learning_rate=learning_rate)
    elif learn_rule == 'adam':
...
# eveupdates(tf.trainableparams, loss)
  else:
    train_op = None
...
def resnet_main(flags, model_function, input_function):
  ...
  classifier = tf.estimator.Estimator(
	  ...
	  'learning_rate': flags.learning_rate,
          'learning_rule': flags.learning_rule
      })
...
class ResnetArgParser(argparse.ArgumentParser):
  ...
  def __init__(self, resnet_size_choices=None):
    ...
    self.add_argument(
        ...
        help='Learning rate for the rule chosen')

    self.add_argument(
        '--learning_rule', type=str, choices=['adam', 'sgd'],
        help='Choose the learning rule to use when training')
